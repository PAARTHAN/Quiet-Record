...
def send_warning_email(user: User) -> tuple[bool, str]:
    subject = f"Quiet Record inactivity warning for {user.name}"
    body = (
        f"Hello {user.name},\n\n"
        f"We noticed that you have not checked in to the Quiet Record application recently and we're not sure whether you're okay!.\n"
